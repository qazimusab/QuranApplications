...
public class DownloadService extends Service {
    ...
    private class DownloadTask extends AsyncTask<Void, Double, Void> {
		...
		protected Void doInBackground(Void... params) {
				...
				//for(int i=1;i<560;i++){
				try {
					downloadFile("http://www.qaziconsultancy.com/Quran/zip_folder/13linequran.zip",Environment.getExternalStorageDirectory().getPath() +"/Pictures/13linequran.zip");
    ...
    private class ExtractTask extends AsyncTask<Void, Double, Void> {
		...
		Intent notificationIntent = new Intent(DownloadService.this, ViewPagerActivity.class);
		notificationIntent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
		PendingIntent contentIntent = PendingIntent.getActivity(DownloadService.this, 0, notificationIntent, 0);
		Notification notification = new Notification(icon, tickerText, when);
		notification.setLatestEventInfo(context, contentTitle, contentText, null);
		...
		protected Void doInBackground(Void... params) {
			...
			try {
				u.unzip(Environment.getExternalStorageDirectory().getPath() +"/Pictures/13linequran.zip", Environment.getExternalStorageDirectory().getPath() +"/Pictures/.13linequran/");
